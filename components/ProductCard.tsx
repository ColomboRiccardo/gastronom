...
const ProductCard = ({product, index}: { product: ProductType, index: number }) => {
    ...
    return (
        <VerticalAnimatedDiv delay={index * 0.05}
                    ...
                    <div className="flex items-end justify-between">
                        <div>
                            <div className="flex items-baseline gap-2">
                                {product.discount > 0 &&
                                    <span
                                        className="text-2xl font-bold text-red-900">{product.originalPrice * (1 - product.discount)}€</span>
                                }
                                <span className="text-sm text-gray-400 line-through">{product.originalPrice}€</span>
                            </div>
                            <span className="text-xs text-gray-500">{product.weightInGrams}gr</span>
                        </div>
